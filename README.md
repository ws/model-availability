...
- **Drift detection**: new models are free, but a previously-committed model vanishing upstream fails the run for human review (`--allow-drift` to accept). Azure additionally re-fetches once before trusting a disappearance, since its API occasionally flickers.
