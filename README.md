...
- **Model removals are recorded directly**: when a provider no longer returns a model, the next snapshot removes it.
- **Creation-timestamp drift acknowledgement, phone-friendly**: OpenAI and Anthropic treat material changes to their immutable creation timestamps as drift. The failing run commits a `<snapshot>.pending-drift.json` marker; re-running the failed workflow or dispatching it with `accept_pending` accepts exactly that recorded timestamp drift. `--allow-drift` accepts all timestamp drift unconditionally.
