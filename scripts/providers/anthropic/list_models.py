...
def main() -> int:
    ...
    # created_at should be immutable. Pin it to the committed value within 24h
    # (ignoring harmless reformatting); a larger jump fails for human review.
    created_drift = reconcile_created(models, read_existing_created(args.output))
    if created_drift and not args.allow_drift:
        print(
            f"error: {len(created_drift)} committed model(s) changed created_at by "
